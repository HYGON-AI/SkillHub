...
description: Create, review, and onboard portable Agent Skills into HYGON-AI SkillHub. Use when adding a new SKILL.md to the catalog, registering a local or remote component in components.d, preparing a SkillHub contribution, or diagnosing catalog validation and synchronization failures.
license: Apache-2.0
...
8. For a remote component only, apply synchronization separately after reviewing destinations, then rerun `contribute.py check`. Remote provenance verifies the registered identity, locked commit, source digest and published tree. Plain `sync_sources.py --check` detects newer upstream revisions; `--check --locked` verifies the reviewed snapshot without following later upstream changes.
