...
Run commands from a HYGON-AI Agent Skills checkout, not from the original skill directory.
...
Import always asks for the **catalog maintaining team** unless `--owner` is
...
Updating the catalog's scripts does not rewrite a previously generated Skill Card.
...
- **Create a new skill in a source repository:** use the authoring steps below,
  merge the source change first, then submit the catalog import PR.
...
new_skill.py --repo ...
  ...
  -> commit --signoff, open the catalog pull request
```
...
resolved commit and the source-tree SHA-256 digest. Open the catalog pull
